...
for i=1:N,
  ...
  if 1,
            ...
            % XXX [gordon] bugfix 18 Oct 2010
            % added code here to remove crash when examining region of constant
            % intensity.
            if ~isfinite(xc(i,1)) || ~isfinite(xc(i,2))
               %xc(i,:) = xt(i,:) + 10*[wintx winty];
               xc(i,:) = xt(i,:);
            end
            % XXX
      cIx = xc(i,1); 			%
